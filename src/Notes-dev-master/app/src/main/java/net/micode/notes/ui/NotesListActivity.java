...
/**
 * 笔记列表主界面活动类
 * 该类是应用的主要入口，负责管理和显示笔记列表，提供笔记的增删改查等核心功能。
 * 
 * 主要功能：
 * 1. 显示笔记和文件夹列表
 * 2. 提供笔记的创建、编辑、删除功能
 ...
 * 
 * 实现的接口：
 * - OnClickListener: 处理视图点击事件
 * - OnItemLongClickListener: 处理列表项长按事件
 * - NotesListItem.OnClickListener: 处理笔记列表项点击事件
 * 
 * @author MiCode
 * @version 1.0
 */
...
public class NotesListActivity extends Activity implements OnClickListener, OnItemLongClickListener, NotesListItem.OnClickListener {
    /** 查询标记常量 */
    private static final int FOLDER_NOTE_LIST_QUERY_TOKEN = 0;  // 文件夹中笔记列表查询的标记
    private static final int FOLDER_LIST_QUERY_TOKEN = 1;       // 文件夹列表查询的标记
    ...
    private static final int SORT_BY_CREATED_DATE = 1;         // 按创建日期排序
    private static final int SORT_BY_TITLE = 2;                // 按标题排序

    /** 对话框常量 */
    private static final int DIALOG_EMPTY_TRASH = 1;           // 清空回收站对话框ID
    private static final int DIALOG_RESTORE_TRASH = 2;         // 恢复回收站对话框ID
    ...
    private void setAppInfoFromRawRes() {
        ...
        if (!sp.getBoolean(PREFERENCE_ADD_INTRODUCTION, false)) {
            ...
            // 使用try-with-resources自动管理多个资源的关闭
            try (InputStream in = getResources().openRawResource(R.raw.introduction);
                ...
                Log.e(TAG, "Introduction resource file not found");
                return;
            } catch (IOException e) {
                Log.e(TAG, "Error reading introduction file");
                return;
            }
...
//                    NoteColumns.SNIPPET,
    ...
    public void onBackPressed() {
        switch (mState) {
            ...
            case NOTE_LIST:
                ...
                super.onBackPressed();
                break;
            case TRASH_FOLDER:    // 添加回收站的处理分支
                // 从回收站状态返回到根文件夹的笔记列表状态
                mCurrentFolderId = Notes.ID_ROOT_FOLDER;
                mState = ListEditState.NOTE_LIST;
                startAsyncNotesListQuery();
                mTitleBar.setVisibility(View.GONE);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.menu_trash:
                Intent intent = new Intent(this, TrashNotesActivity.class);
    ...
    @Override
    public void onClickItem(NoteItemData item) {
        if (item == null)
            return;
        ...
        Intent intent = new Intent(this, NoteEditActivity.class);
        intent.setAction(Intent.ACTION_VIEW);
        intent.putExtra(Notes.INTENT_EXTRA_NOTE_ID, item.getId());
        
        // 如果是从回收站打开的便签，添加标记
        if (item.getParentId() == Notes.ID_TRASH_FOLER) {
            intent.putExtra(Notes.INTENT_EXTRA_FROM_TRASH, true);
        }
