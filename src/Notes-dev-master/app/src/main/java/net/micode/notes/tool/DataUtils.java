...
public class DataUtils {
    ...
    private static final String LOG_IDS_NULL = "the ids is null";
    private static final String LOG_IDS_EMPTY = "no id is in the hashset";
    private static final String LOG_DELETE_FAILED = "delete notes failed, ids: %s";
    private static final String LOG_SYSTEM_FOLDER = "Don't delete system folder root";

    /**
     * 私有构造函数，防止实例化
     */
    private DataUtils() {
        throw new AssertionError("DataUtils class is utility class and cannot be instantiated");
    }
    ...
    public static boolean batchDeleteNotes(ContentResolver resolver, Set<Long> ids) {
        if (ids == null) {
            Log.d(TAG, LOG_IDS_NULL);
            return true;
        }
        if (ids.isEmpty()) {
            Log.d(TAG, LOG_IDS_EMPTY);
            return true;
        }
        ...
        try {
            ...
            // 检查删除结果
            if (results.length == 0) {
                Log.d(TAG, String.format(LOG_DELETE_FAILED, ids));
                return false;
            }
            return true;
        } catch (RemoteException | OperationApplicationException e) {
            Log.e(TAG, String.format(ERROR_FORMAT, e.getClass().getSimpleName(), e.getMessage()));
            return false;
        }
    }
     ...
     */
    public static boolean batchMoveToFolder(ContentResolver resolver, Set<Long> ids,
                                            long folderId) {
        if (ids == null) {
            Log.d(TAG, LOG_IDS_NULL);
            return true;
        ...
        try {
            ...
            // 检查移动结果
            if (results.length == 0) {
                Log.d(TAG, String.format("move notes failed, ids: %s", ids));
                return false;
            }
            return true;
        } catch (RemoteException | OperationApplicationException e) {
