    ...
    public int getSyncAction(Cursor c) {
        try {
            ...
            if (c.getInt(SqlNote.LOCAL_MODIFIED_COLUMN) == 0) {
                // 无本地更新，检查是否有远程更新
                return (c.getLong(SqlNote.SYNC_ID_COLUMN) == getLastModified())
                    ? SYNC_ACTION_NONE        // 双方均无更新
                    : SYNC_ACTION_UPDATE_LOCAL; // 应用远程更新到本地
            } 
            ...
            // 对于有本地修改的情况，始终更新远程
            // 这包括：仅本地有修改 和 发生冲突时以本地修改为准
            return SYNC_ACTION_UPDATE_REMOTE;
            
        } catch (Exception e) {
            Log.e(TAG, "Error in getSyncAction", e);
            return SYNC_ACTION_ERROR;
        }
    }
